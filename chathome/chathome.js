...
//         console.log("hi")
//         localStorage.setItem("chats",undefined)
...
console.log(username)
    ...
    const response = await axios.post("http://localhost:1000/group/deleteuser",obj, { headers: { "Authorization": localStorage.getItem("token") } })
...
async function makehimadmin(event){
    ...
    const response = await axios.post("http://localhost:1000/group/superuser",obj, { headers: { "Authorization": localStorage.getItem("token") } })
    .then(response => { document.getElementById("resultprinting").innerText=response.data.message }).catch((err)=> { return err})
    console.log(response)
...
groupid
    ...
    const response = await axios.post("http://localhost:1000/group/fetchuser", id)
.then(response => response)
